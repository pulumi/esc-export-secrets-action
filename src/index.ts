...
async function ensureEnvironment(
    ...
    try {
        await client.rawApi.createEnvironment(org, { project, name });
    } catch (error: any) {
        if (error.response && error.response.status === 409) {
            return;
        }
        throw new Error(
            `Failed to create environment: ${error.response.statusText}`,
        );
    }
}
...
async function run(): Promise<void> {
    try {
        ...
        const excludeSecrets = new Set(
            (core.getInput('exclude-secrets') || '')
                .split(',')
                .map(s => s.trim()),
        );
        ...
        if (orgYaml !== undefined || repoYaml !== undefined) {
            core.info(
                `creating environment ${orgProject}/${orgEnvironment} if necessary`,
            );
        ...
        if (orgYaml !== undefined) {
            core.info(`updating environment ${orgProject}/${orgEnvironment}`);
        ...
        if (repoYaml !== undefined) {
            core.info(
                `creating environment ${repoProject}/${repoEnvironment} if necessary`,
            );
            await ensureEnvironment(
                escClient,
                organization,
            ...
            core.info(`updating environment ${repoProject}/${repoEnvironment}`);
            const diags = await escClient.updateEnvironmentYaml(
                organization,
                repoProject,
                repoEnvironment,
